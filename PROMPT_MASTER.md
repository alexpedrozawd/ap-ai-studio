...
Este documento contém o prompt master definitivo para a criação de um pipeline de geração de vídeo e efeitos visuais em um servidor **Bazzite 44 (Fedora Kinoite, imutável/ostree)** com **AMD Radeon RX 9070 XT (RDNA4)**, mantendo a integridade do sistema operacional.

> **Nota histórica.** Este projeto nasceu em Ubuntu 24.04 com NVIDIA RTX 5060 Ti e migrou em
> 2026-07-26 (ver [`MIGRACAO_BAZZITE.md`](MIGRACAO_BAZZITE.md)). As seções de especificação
> abaixo refletem o ambiente **atual**. Os blocos marcados como "Nota de execução" são
> registro histórico do que de fato aconteceu na época e foram preservados de propósito —
> reescrevê-los apagaria as lições que justificam decisões que continuam valendo.
...
**Ambiente:** Bazzite 44 / Fedora Kinoite (Wayland, **imutável/ostree**) | **Hardware:** Radeon RX 9070 XT 16GB (RDNA4/gfx1201, driver `amdgpu`), 32GB RAM, SSD NVMe M.2 (OS + dados do pipeline).
**Restrição de sistema imutável:** a raiz é somente-leitura e `rpm-ostree` exige reboot. Todo o stack de IA vive num contêiner **distrobox** com acesso a `/dev/dri` e `/dev/kfd` — nada é instalado no sistema base, e o ROCm do contêiner não substitui o driver do host.
**Restrição de Servidor Multitarefa:** o PC também é desktop KDE e máquina de jogos (Steam). A GPU é compartilhada. NENHUMA trava de atualização de SO/drivers é permitida.
...
1. **Portões de Autorização (CRÍTICO):** Antes de tarefas críticas, o script DEVE pausar e exigir `[Y/n]` no terminal:
   ...
   * **Gate 3 - I/O de Disco (VAAPI Chunking):** Antes do FFmpeg usar `h264_vaapi` e gravar os arquivos intermediários no formato *ffv1* ou *libx264 -crf 0* em `/var/home/apsrv/ai_pipeline` (NVMe compartilhado com o SO). Checar espaço livre em `AP_AI_STUDIO_HOME` antes de iniciar (**nunca em `/`**: no ostree a raiz é composefs somente-leitura e mede sempre ~0 livre) e abortar se estiver abaixo da margem de segurança, já que não há mais um disco isolado para conter o consumo.
...
4. **`PYTORCH_HIP_ALLOC_CONF=expandable_segments:True`:** definir essa variável de ambiente no processo do ComfyUI para reduzir OOM por fragmentação de memória (quando cabe nominalmente mas falta um bloco contíguo).
...
(`<ip-tailscale-do-servidor>:8299`, nunca `0.0.0.0`, sem `ufw allow`). Documentação operacional
...
13. **`--blocks-to-swap` (avançado, opcional) no modo `video`** — experimento real de
    ...
    sozinhos, mais que os 16GB da RX 9070 XT, então algum grau de offload pra CPU é
...
é o que cabe com folga nos 16GB de VRAM da RX 9070 XT sem risco de OOM durante a
...
viver fisicamente dentro de `/var/home/apsrv/ap-ai-studio/` (2026-07-05, pedido do usuário):**
...
`MANUAL_USO.md`) passou a apontar direto pra `/var/home/apsrv/ap-ai-studio/...`, sem
...
`/var/home/apsrv/ai_pipeline` no texto original das Fases 0/1/3 acima descrevem a decisão de
...
`/var/home/apsrv/ap-ai-studio/ai_pipeline` (ver Referência rápida abaixo).

**Achado real #24b — o grep inicial da consolidação perdeu o padrão `~/miniconda3` (só
cobria `/var/home/apsrv/miniconda3` literal), quebrando FaceFusion/TTS/Demucs/ComfyUI de verdade
...
(`MINICONDA_DIR = "/var/home/apsrv/ap-ai-studio/miniconda3"`) — centraliza o valor numa
...
*Modelos baixados (`/var/home/apsrv/ap-ai-studio/ai_pipeline/models_hub/models/` — caminho atualizado no achado #24, ~42.6GB total confirmado por `du -h` em 2026-07-02):*
...
- Disco (`/`, NVMe compartilhado com o SO): ~224,9GB livres de 468GB em 2026-07-03 (após
  ...
  Battle.net/World of Warcraft (`/var/home/apsrv/Games/battlenet`, 123GB) e do Lutris (dados de
